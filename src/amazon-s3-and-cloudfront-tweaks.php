...
class Amazon_S3_and_CloudFront_Tweaks {
   ...
   */
    ...
    /**
     * This filter allows you to change the endpoint used for talking to the api.
     *
     * @return string
     *
     * MinIO endpoint, like Immortals in Highlander, there can be only one.
     */
    function minio_get_host() {
        $host = '127.0.0.1';
        if(getenv('MINIO_HOST')) {
     ...
     * MinIO endpoint, like Immortals in Highlander, there can be only one.
     */
    function minio_get_port() {
        $port = '54321';
        if(getenv('MINIO_PORT')) {
     ...
     * MinIO endpoint, like Immortals in Highlander, there can be only one.
     */
    function minio_get_proto() {
        $proto = 'http';
        if(getenv('MINIO_PROTO')) {
            $proto = getenv('MINIO_PROTO');
        }
        return $proto;
    }
    ...
    function minio_s3_client_args( $args ) {
        ...
        $args['endpoint'] = $this->minio_get_proto() . '://' . $this->minio_get_host() . ':' . $this->minio_get_port();
    ...
    function minio_s3_url_domain( $domain, $bucket, $region, $expires, $args, $preview ) {
        ...
        if(getenv('MINIO_URL_DOMAIN')) {
            $domain = getenv('MINIO_URL_DOMAIN');
        } else {
            $domain = $this->minio_get_host() . ':' . $this->minio_get_port();
        }
    ...
    function minio_s3_console_url( $url ) {
        ...
        return $this->minio_get_proto() . '://' . $domain . '/minio/';
    }
    /**
